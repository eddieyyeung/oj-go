...
func (st *SegmentTree) Query(node *SegmentTreeNode, start int, end int, left int, right int) int {
	...
	// [mid + 1, end] 和 [l, r] 可能有交集，遍历右孩子区间
	if right > mid {
		ans = max(st.Query(node.Right, mid+1, end, left, right), ans)
	}
	// ans 把左右子树的结果都累加起来了，与树的后续遍历同理
	return ans
}

func (st *SegmentTree) UpdateOne(index int, val int) {
	st.Update(st.Head, 0, st.Size-1, index, index, val)
}
...
func (mc *MyCalendarTwo) Book(start int, end int) bool {
	if mc.ST.Query(mc.ST.Head, 0, mc.ST.Size-1, start, end-1) == 2 {
		return false
	}
	mc.ST.Update(mc.ST.Head, 0, mc.ST.Size-1, start, end-1, 1)
	return true
}
